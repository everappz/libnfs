...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        // 1. Local-only tests (no server needed)
        testClientInit();
        testFileItemInit();

        // 2. Create client for server tests
        NSURL *url = [NSURL URLWithString:kServerURL];
        LNFSClient *client = [[LNFSClient alloc] initWithURL:url];
        client.timeout = 30.0;
        client.uid = 0;
        ...
        // 3. List exports
        testListExports(client);

        // 4. Connect
        testConnect(client);

        // 5. Directory operations (creates testDir)
        testDirectoryOperations(client, testDir);

        // 6. File write/read
        testFileWriteAndRead(client, testDir);

        // 7. Stat operations (uses file from step 6)
        testStatOperations(client, testDir);

        // 8. Filesystem stats
        testStatvfs(client);

        // 9. Truncate
        testTruncate(client, testDir);

        // 10. Rename
        testRename(client, testDir);

        // 11. Unlink
        testUnlink(client, testDir);

        // 12. RemoveItem
        testRemoveItem(client, testDir);

        // 13. Recursive directory operations
        testRecursiveDirectory(client, testDir);

        // 14. Large file I/O
        testLargeFileReadWrite(client, testDir);

        // 15. Upload / Download
        testUploadDownload(client, testDir);

        // 16. Large Upload / Download (50 MB)
        testLargeUploadDownload(client, testDir);

        // 17. Disconnect and reconnect
        testDisconnectAndReconnect(client);

        // 17. Error handling
        testErrorCases(client);

        // 18. Cleanup test directory
        GROUP(@"Cleanup");
        __block NSError *cleanupError = nil;
        waitForCompletion(^(dispatch_semaphore_t sem) {
            [client removeItemAtPath:testDir completion:^(NSError *error) {
                cleanupError = error;
                dispatch_semaphore_signal(sem);
            }];
        });
        if (cleanupError) {
            // Also try removing the file that testFileWriteAndRead leaves behind
            waitForCompletion(^(dispatch_semaphore_t sem) {
                [client removeFileAtPath:[testDir stringByAppendingPathComponent:@"testfile.txt"]
                              completion:^(NSError *error) {
                    dispatch_semaphore_signal(sem);
                }];
            });
            waitForCompletion(^(dispatch_semaphore_t sem) {
                [client removeDirectoryAtPath:testDir recursive:YES completion:^(NSError *error) {
                    dispatch_semaphore_signal(sem);
                }];
            });
        }
